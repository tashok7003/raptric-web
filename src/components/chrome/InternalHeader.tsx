...
export function InternalHeader({
  ...
  return (
    <header className="border-b border-[var(--color-border)] bg-ink px-4 py-3 text-white">
      <div className="mx-auto flex max-w-6xl items-center gap-6">
        <span className="font-display text-[15px] font-bold">RAPTRIC · {tool}</span>
        <nav className="flex items-center gap-4">
          {links.map((l) => (
            <Link key={l.href} href={l.href} className="text-[13px] text-[#b8b1a6] hover:text-white">
              {l.label}
            </Link>
          ))}
        </nav>
        <span className="ml-auto rounded-[999px] border border-[#4a453d] px-2 py-0.5 text-[11px] text-[#b8b1a6]">
          {role}
        </span>
      </div>
    </header>
